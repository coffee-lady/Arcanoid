...
function EasingsLib:out_in_quad(time, start, change, duration)
    if time < duration / 2 then
        return self:out_quad (time * 2, start, change / 2, duration)
    else
        return self:in_quad((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:out_in_cubic(time, start, change, duration)
    if time < duration / 2 then
        return self:out_cubic(time * 2, start, change / 2, duration)
    else
        return self:in_cubic((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:out_in_quart(time, start, change, duration)
    if time < duration / 2 then
        return self:out_quart(time * 2, start, change / 2, duration)
    else
        return self:in_quart((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:out_in_quint(time, start, change, duration)
    if time < duration / 2 then
        return self:out_quint(time * 2, start, change / 2, duration)
    else
        return self:in_quint((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:out_in_sine(time, start, change, duration)
    if time < duration / 2 then
        return self:out_sine(time * 2, start, change / 2, duration)
    else
        return self:in_sine((time * 2) -duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:in_out_expo(time, start, change, duration)
    ...
    if time == duration then return start + change end
    time = time / duration * 2
    if time < 1 then
...
function EasingsLib:out_in_expo(time, start, change, duration)
    if time < duration / 2 then
        return self:out_expo(time * 2, start, change / 2, duration)
    else
        return self:in_expo((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:out_in_circ(time, start, change, duration)
    if time < duration / 2 then
        return self:out_circ(time * 2, start, change / 2, duration)
    else
        return self:in_circ((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
...
function EasingsLib:in_elastic(time, start, change, duration, amplitude, period)
    ...
    if time == 1  then return start + change end

    if not period then period = duration * 0.3 end
...
function EasingsLib:in_out_elastic(time, start, change, duration, amplitude, period)
    ...
    if time == 2 then return start + change end

    if not period then period = duration * (0.3 * 1.5) end
...
function EasingsLib:out_in_elastic(time, start, change, duration, amplitude, period)
    if time < duration / 2 then
        return self:out_elastic(time * 2, start, change / 2, duration, amplitude, period)
    else
        return self:in_elastic((time * 2) - duration, start + change / 2, change / 2, duration, amplitude, period)
    end
end
...
function EasingsLib:out_in_back(time, start, change, duration, s)
    if time < duration / 2 then
        return self:out_back(time * 2, start, change / 2, duration, s)
    else
        return self:in_back((time * 2) - duration, start + change / 2, change / 2, duration, s)
    end
end
...
function EasingsLib:in_bounce(time, start, change, duration)
    return change - self:out_bounce(duration - time, 0, change, duration) + start
end

function EasingsLib:in_out_bounce(time, start, change, duration)
    if time < duration / 2 then
        return self:in_bounce(time * 2, 0, change, duration) * 0.5 + start
    else
        return self:out_bounce(time * 2 - duration, 0, change, duration) * 0.5 + change * .5 + start
    end
end

function EasingsLib:out_in_bounce(time, start, change, duration)
    if time < duration / 2 then
        return self:out_bounce(time * 2, start, change / 2, duration)
    else
        return self:in_bounce((time * 2) - duration, start + change / 2, change / 2, duration)
    end
end
