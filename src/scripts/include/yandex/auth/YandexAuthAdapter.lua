...
local YandexAuthAdapter = {}

YandexAuthAdapter.IMAGE_SIZE = {
    small = 'small',
    medium = 'medium',
    large = 'large',
}

function YandexAuthAdapter:init()
    return Async(function(done)
        yagames.player_init(nil, function(_, err)
...
function YandexAuthAdapter:get_environment()
    return yagames.environment()
end

function YandexAuthAdapter:authenticate(on_resolve, on_reject)
    local user_logged = false

    Async(function(done)
        yagames.auth_open_auth_dialog(function(_, err)
...
function YandexAuthAdapter:get_current_user_photo_async(images_url, image_size)
    local succeeded, url = pcall(yagames.player_get_photo, image_size or YandexAuthAdapter.IMAGE_SIZE.small)
...
function YandexAuthAdapter:get_user_photo_async(user_img_url, images_url)
