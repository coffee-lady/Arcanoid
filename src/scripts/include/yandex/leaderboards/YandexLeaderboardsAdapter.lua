...
local YandexLeaderboardsAdapter = {}

YandexLeaderboardsAdapter.IMAGE_SIZE = {
    small = 'small',
    medium = 'medium',
    large = 'large',
}

function YandexLeaderboardsAdapter:init_async()
    Async(function(done)
        yagames.leaderboards_init(function(_, err)
...
function YandexLeaderboardsAdapter:get_leaderboard_config_async(id)
    return Async(function(done)
        yagames.leaderboards_get_description(id, function(_, err, data)
...
function YandexLeaderboardsAdapter:get_player_entry_async(leaderboard_id, options)
    return Async(function(done)
        yagames.leaderboards_get_player_entry(leaderboard_id, options and {
            getAvatarSrc = options.avatar_size,
        } or {}, function(_, err, result)
...
-- }
function YandexLeaderboardsAdapter:get_leaderboard_async(leaderboard_id, options)
    return Async(function(done)
        yagames.leaderboards_get_entries(leaderboard_id, options and {
            includeUser = options.include_user,
            quantityAround = options.amount_around,
            quantityTop = options.amount_top,
            getAvatarSrc = options.avatar_size,
        } or {}, function(_, err, result)
            debug_logger:log('got leaderboard entries', leaderboard_id, debug_logger:inspect({
                includeUser = options.include_user,
                quantityAround = options.amount_around,
                quantityTop = options.amount_top,
                getAvatarSrc = options.avatar_size,
...
function YandexLeaderboardsAdapter:set_score_async(leaderboard_id, score)
    return Async(function(done)
        yagames.leaderboards_set_score(leaderboard_id, score, nil, function(_, err)
...
return YandexLeaderboardsAdapter
