...
local YandexInterstitialAdsAdapter = {}

function YandexInterstitialAdsAdapter:init_timer(delay, file, key_timer, player_data_storage)
...
function YandexInterstitialAdsAdapter:show(callbacks)
    if not self.timer:is_expired() then
        debug_logger:log('timer is not expired. seconds_left:', YandexInterstitialAdsAdapter.timer.seconds_left)
        return
    end
...
function YandexInterstitialAdsAdapter:show_with_probability(probability, callbacks)
    callbacks = callbacks or {}
...
function YandexInterstitialAdsAdapter:resume_timer()
    self.timer:resume()
end

return YandexInterstitialAdsAdapter
