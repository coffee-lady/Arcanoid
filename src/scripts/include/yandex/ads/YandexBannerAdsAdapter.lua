...
local YandexBannerAdsAdapter = {}

function YandexBannerAdsAdapter:init_async(banner_height)
    self.banner_height = banner_height

    Async(function(done)
        yagames.banner_init(function(_, err)
...
function YandexBannerAdsAdapter:load_async(params)
    ...
    Async(function(done)
        yagames.banner_create(id, {
            css_class = css_string,
            css_styles = String.replace_vars('height: {height}px; width: {width}px; transform-origin: bottom; display: none;', YandexBannerAdsAdapter._get_banner_params()),
        }, function(_, err, _)
            self.initted = true
...
function YandexBannerAdsAdapter:_get_banner_params()
    if not html5 then
        return {
            height = 0,
            width = 0,
...
function YandexBannerAdsAdapter:show(id)
...
function YandexBannerAdsAdapter:hide(id)
    if not self.initted then
        debug_logger:log('ERROR: trying show banner: banner is not initted')
        return
    end
...
function YandexBannerAdsAdapter:refresh(id)
    if not self.initted then
        debug_logger:log('ERROR: trying to refresh: banner is not initialized')
        return
    end

    yagames.banner_refresh(id, function(_, err)
