...
local BEFORE_SCENE_CHANGE = hash('before_scene_change')

ScenesService.SCENE_CHANGE = SCENE_CHANGE
ScenesService.BEFORE_SCENE_CHANGE = BEFORE_SCENE_CHANGE
...
function ScenesService:_notify()
    ...
    self.scene_change_notifier:emit({
...
function ScenesService:_notify_before(current_scene, scene_data)
    ...
    self.before_scene_change_notifier:emit({
        current_scene = current_scene,
        prev_scene = prev_scene,
...
function ScenesService:subscribe()
    self.scene_change_notifier:subscribe()
    self.before_scene_change_notifier:subscribe()
end
