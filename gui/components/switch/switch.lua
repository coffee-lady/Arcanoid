...
function Switch:initialize(context_services, params)
    --- @type EventBus
    self.event_bus = context_services.event_bus
    --- @type ScenesService
    self.scenes_service = context_services.scenes_service

    self.view = View(context_services, params.ID, params.enabled)
...
function Switch:switch(action)
    ...
    self.scenes_service:post_to_go(BOOTSTRAP_URL, Switch.SWITCH_TOGGLED, {
