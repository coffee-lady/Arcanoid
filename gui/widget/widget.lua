...
function Widget:initialize(context_services, is_common)
    --- @type EventBus
...
function Widget:set_theme_map(map, extra_theme_key)
    self.theme_map = Core.ThemeMap(self.ui_service, self.is_common, extra_theme_key)
    self.theme_map:init(map)
...
function Widget:subscribe_on_refresh_theme(event_id)
    self.event_bus:on(event_id, self.theme_map.refresh, self.theme_map)
end
...
function Widget:create_button(ids, callback, type_id)
    local btn = Controls.SubscribedButton(ids, self.context_services, callback)
...
function Widget:create_link(ids, callback, type_id)
    local btn = Controls.SubscribedLink(ids, self.context_services, callback)
...
function Widget:set_buttons_menu(map)
    self.buttons = Controls.ButtonsMenu(self.context_services, map)
    return self.buttons
end

function Widget:set_flow_buttons_menu(map)
    self.buttons = Controls.FlowButtonsMenu(self.context_services, map)
    return self.buttons
end
...
function Widget:set_links_menu(map)
    self.links = Controls.LinksMenu(self.context_services, map)
    return self.links
end
...
function Widget:set_localization(key, map)
    self.localization_map = LocalizationMap(self.context_services, key, map)
end
