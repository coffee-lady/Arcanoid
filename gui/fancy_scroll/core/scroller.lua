...
function Scroller:update_auto_scroll(dt)
    ...
    if self.autoscroll.elastic then
        position, self.scroll_velocity = Math.smooth_damp(self.current_position, self.current_position + offset, self.scroll_velocity, self.scroll_elasticity, Math.INFINITY, dt)
        ...
        local alpha = Math.clamp01((socket.gettime() - self.autoscroll.start_time) / math.max(self.autoscroll.duration, 0.0001))
        position = Math.lerp_unclamped(self.scroll_start_position, self.autoscroll.position_end, self.autoscroll.easing_func(alpha))
...
function Scroller:update_velocity(dt)
    ...
    if not is_offset_zero then
        ...
        self:update_selection(Math.clamp(Math.round(position), 0, self.total_count - 1), position)
...
function Scroller:on_pointer_up()
    self.is_process_pointer = false

    if self.is_hold and self.is_snap then
        self:update_selection(Math.clamp(Math.round(self.current_position), 0, self.total_count - 1), self.current_position)
        self:scroll_to(Math.round(self.current_position), self.snap_duration, self.snap_easing)
    end
...
function Scroller:scroll_to(position, duration, easing_func, complete_callback)
    ...
    self.autoscroll.position_end = self.current_position + self:calculate_movement_amount(self.current_position, position);
    ...
    self:update_selection(Math.round(Math.circular_position(self.autoscroll.position_end, self.total_count)), self.current_position)
end
...
function Scroller:jump_to(index)
    ...
    if index <= self.bottom_offset then
        return
    end
    ...
    self:update_selection(index, self.current_position)
...
function Scroller:calculate_movement_amount(from_position, to_position)
    ...
    local amount = Math.circular_position(to_position, self.total_count) - Math.circular_position(from_position, self.total_count);
